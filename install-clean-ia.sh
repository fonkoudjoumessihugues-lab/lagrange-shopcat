#!/bin/bash
echo "📦 Installation de l'IA propre..."

# Trouver où insérer l'IA (avant CONFIGURATION FACTURE)
LINE=$(grep -n "CONFIGURATION FACTURE" server.js | cut -d: -f1)

if [ -z "$LINE" ]; then
  echo "⚠️ CONFIGURATION FACTURE non trouvé, insertion à la fin"
  LINE=$(wc -l < server.js)
fi

# Créer le fichier avec l'IA
head -n $((LINE - 1)) server.js > server.js.new

# Ajouter l'IA propre
cat >> server.js.new << 'NEWIA'
...
# Ajouter la suite du fichier
tail -n +$LINE server.js >> server.js.new

# Remplacer
mv server.js.new server.js

echo "✅ IA propre installée !"
echo ""
