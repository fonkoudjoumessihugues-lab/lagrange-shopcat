#!/bin/bash
echo "🔍 Recherche de la section IA..."

# Chercher la ligne contenant app.post('/api/ai/ask'
LINE=$(grep -n "app.post('/api/ai/ask" server.js | cut -d: -f1)

if [ -n "$LINE" ]; then
  echo "✅ Section IA trouvée à la ligne $LINE"
  
  # Créer une sauvegarde
  cp server.js server.js.ia.backup
  
  # Supprimer l'ancienne IA et insérer la nouvelle
  node -e "
  const fs = require('fs');
  let code = fs.readFileSync('server.js', 'utf8');
  
  // Trouver le début de la fonction IA
  const startMatch = code.match(/\/\/ ========== IA .*?==========/);
  const endMatch = code.match(/\/\/ ========== CONFIGURATION FACTURE ==========/);
  
  if (startMatch && endMatch) {
    const start = startMatch.index;
    const end = endMatch.index;
    const newIA = \`// ========== IA SIMPLIFIÉE ==========
app.post('/api/ai/ask', auth, (req, res) => {
  try {
    ...
    code = code.substring(0, start) + newIA + '\n\n' + code.substring(end);
    fs.writeFileSync('server.js', code);
    console.log('✅ IA remplacée avec succès !');
  } else {
  ...
  echo "❌ Aucune section IA trouvée dans server.js"
  echo "📝 Vérifie que le fichier contient app.post('/api/ai/ask'"
fi
